                    ...
                    <table class="w-full border border-gray-300 border-collapse">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-300">
                                ...
                                <td class="py-3 px-6 border border-gray-300">{{ $product->category->category_name }}</td>
                                <td class="py-3 px-6 border border-gray-300">{{ $product->task_date ? \Carbon\Carbon::parse($product->task_date)->format('F j, Y') : 'N/A' }}</td>
                                <td>
                                    <div>
                                    ...
                                    <form action="{{ route('product.comment', $product->product_id) }}" method="POST" class="mt-2">
                                        @csrf
                                        <input type="text" name="comment" placeholder="Add a comment" class="border border-gray-300 rounded px-2 py-1 w-full center" />
                                        <button type="submit" class="bg-blue-500 text-white rounded px-2 py-1 mt-1">Submit</button>
                                    </form>
                                </td>
                                <td class="py-3 px-6 border border-gray-300 action-cell">
                                    <a title="EDIT" href="{{ route('product.edit', $product->product_id) }}" class="action-button edit-button">
                                        EDIT
                                    </a>
                                    <form method="POST" action="{{ route('product.delete', $product->product_id) }}" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            type="submit"
